/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 3.0.12
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package ch.ehi.fgdb4j.jni;

public enum CurveType {
  curveTypeCircularArc(1),
  curveTypeBezier(4),
  curveTypeEllipticArc(5);

  public final int swigValue() {
    return swigValue;
  }

  public static CurveType swigToEnum(int swigValue) {
    CurveType[] swigValues = CurveType.class.getEnumConstants();
    if (swigValue < swigValues.length && swigValue >= 0 && swigValues[swigValue].swigValue == swigValue)
      return swigValues[swigValue];
    for (CurveType swigEnum : swigValues)
      if (swigEnum.swigValue == swigValue)
        return swigEnum;
    throw new IllegalArgumentException("No enum " + CurveType.class + " with value " + swigValue);
  }

  @SuppressWarnings("unused")
  private CurveType() {
    this.swigValue = SwigNext.next++;
  }

  @SuppressWarnings("unused")
  private CurveType(int swigValue) {
    this.swigValue = swigValue;
    SwigNext.next = swigValue+1;
  }

  @SuppressWarnings("unused")
  private CurveType(CurveType swigEnum) {
    this.swigValue = swigEnum.swigValue;
    SwigNext.next = this.swigValue+1;
  }

  private final int swigValue;

  private static class SwigNext {
    private static int next = 0;
  }
}

